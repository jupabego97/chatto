...
<EnvVar name="CHATTO_OWNERS_EMAILS" toml="owners.emails">
  Comma-separated list of email addresses that confer instance-owner status. A user with any verified email matching the list gets full instance access (including `/admin` routes). Only verified email addresses are matched.
</EnvVar>
