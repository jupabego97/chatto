...
<Aside type="caution">
  `chatto reset rbac` wipes custom roles, assignments, and permission decisions, then reseeds the defaults. Use it as an operator recovery tool, not as a routine configuration workflow.
</Aside>
