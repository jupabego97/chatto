...
Bot accounts do not use passwords, OAuth, or browser cookie sessions. They authenticate to GraphQL and API upload paths with named bot API tokens whose raw secret is shown only once on creation and starts with `cht_BT`.

Each token has creator-chosen expiry: 30 days, 90 days, 365 days, indefinite, or a custom date. Operators can set `auth.bot_token_max_ttl` to disallow indefinite tokens and cap every preset/custom expiry. Token records live in `RUNTIME_STATE` under HMAC-derived keys with safe metadata such as name, created time, last-used time, expiry, and revocation state; raw token secrets are not written to events, logs, or GraphQL metadata.
...
`RUNTIME_STATE` is included in backups so active sessions and pending registration, email-verification, password-reset, account-deletion, bot-token, and OAuth-code flows can survive a restore. Credential entries in that bucket use HMAC-derived keys, so backup archives do not contain redeemable raw bearer tokens, bot API tokens, links, or OAuth codes. Restoring with a different `core.secret_key` intentionally invalidates those credentials.
...
| Bot API token metadata              | `RUNTIME_STATE` (HMAC-keyed, fixed expiry)  | Yes (not raw tokens) |
| Account-flow link/code verifiers    | `RUNTIME_STATE` (HMAC-keyed, with TTL)      | Yes (not raw links/codes) |
