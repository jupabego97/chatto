...
export class MessagesStore {
  ...
  beginOptimisticReaction(input: {
    ...
    for (let i = 0; i < this.events.length; i++) {
      ...
      if (!updated) continue;
      const key = this.optimisticEventKey(event.id);
      record({ key, previous: event, source: 'events' });
    ...
    for (const [key, event] of this.previewEvents) {
      ...
      const updated = this.eventWithOptimisticReaction(event, input.emoji, input.action);
      if (!updated) continue;
      const versionKey = this.optimisticPreviewKey(key);
      record({ key: versionKey, previous: event, source: 'preview', previewKey: key });
      this.previewEvents.set(key, updated);
    }
  ...
  private optimisticEventKey(eventId: string): string {
    return `events:${eventId}`;
  }

  private optimisticPreviewKey(previewKey: string): string {
    return `preview:${previewKey}`;
  }

  private clearOptimisticVersionForEvent(eventId: string): void {
    this.optimisticReactionVersions.delete(this.optimisticEventKey(eventId));
    this.optimisticReactionVersions.delete(this.optimisticPreviewKey(this.previewKey(eventId)));
  }
  ...
  private applyServerReactionSnapshot(
    ...
    if (snapshot.source === 'events') {
      const index = this.events.findIndex((event) => this.optimisticEventKey(event.id) === snapshot.key);
      if (index === -1) return;
  ...
  private restoreOptimisticReactionSnapshot(snapshot: OptimisticReactionSnapshot): void {
    if (snapshot.source === 'events') {
      const index = this.events.findIndex((event) => this.optimisticEventKey(event.id) === snapshot.key);
      if (index !== -1) this.events[index] = snapshot.previous;
      return;
    }

    if (snapshot.previewKey) this.previewEvents.set(snapshot.previewKey, snapshot.previous);
  }
