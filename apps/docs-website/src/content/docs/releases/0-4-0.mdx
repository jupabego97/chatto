...
- Authenticated server state was split: replace `ServerService.GetServerState` with
  ...
  Admin server responses now return `ServerPublicProfile` directly as
  `public_profile` beside editable config instead of wrapping public metadata and
  MOTD in `ServerProfile`.
...
- Message attachment URL refreshes moved to room-scoped asset reads: replace
  ...
  Asset reads and upload completion now return the first-class `Asset` shape.
...
- Active voice call list reads were renamed from
  `VoiceCallService.ListActiveCallRooms` to `ListActiveCalls`; each `ActiveCall`
  now carries a `RoomSummary` in `room` instead of a scalar `room_id`.
- `includes` maps are intentionally limited to hot paginated timeline/feed paths
  such as room events and followed threads. Singular resource reads and mutations
  return their resources directly, and ordinary list/read clients should use
  `BatchGet*` hydration when they need related resources.
