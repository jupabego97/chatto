...
description: Configure OIDC, GitHub, GitLab, Google, Discord, or AT Protocol sign-in for a Chatto server.
---
...
| `atproto`        | AT Protocol identities, including Bluesky    |
...
AT Protocol is handle-first and uses fixed routes instead of `/auth/providers/{providerID}`:

```txt
https://chat.example.com/auth/atproto
https://chat.example.com/auth/atproto/callback
https://chat.example.com/auth/atproto/client-metadata.json
```

No provider-side client registration is required. Chatto serves its own client metadata document, so non-loopback deployments must use a publicly reachable `webserver.url`.
...
type = "atproto"
label = "AT Protocol"
# request_email defaults to true for AT Protocol.
# auto_provision defaults to true for AT Protocol.
```

AT Protocol sign-in asks the user for their handle first, resolves that handle to the user's PDS, then sends the browser through the PDS OAuth approval flow. The provider ID must be `atproto` because the OAuth metadata and callback routes are fixed.
...
| `atproto`        | `atproto`                           | `atproto`, `account:email`         |
...
`request_email` defaults to `false` for OAuth/OIDC providers and `true` for AT Protocol. Set `request_email = true` to request provider email scopes. For OIDC, the default request is `openid profile`; Chatto can create, link, and sign in accounts from the provider `sub` without receiving an email claim.
...
AT Protocol defaults `auto_provision` to `true` because the handle-first flow is intended as a self-service sign-in path. Set `auto_provision = false` if you only want existing users to link AT Protocol identities.
...
- AT Protocol is matched by the configured `atproto` provider ID plus the user's DID, not their mutable handle.
