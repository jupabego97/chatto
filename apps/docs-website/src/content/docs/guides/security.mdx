...
Login comparison runs bcrypt against a dummy hash for unknown users and passwordless users so the response time is identical regardless of whether the account exists. This blocks the most basic form of user enumeration via timing.
...
| Account-flow, pending OIDC, and OAuth-code verifiers | `RUNTIME_STATE` (HMAC-keyed, with TTL) | Yes (not raw codes or links) |
...
| OIDC provider client secrets | `chatto.toml` `[[auth.providers]]` / `CHATTO_AUTH_PROVIDERS_<index>_CLIENT_SECRET` | n/a |
