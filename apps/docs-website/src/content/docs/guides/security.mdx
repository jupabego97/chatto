...
This boundary is intentional and enforced in the admin API — admin operations do not fetch message bodies. If your community needs message moderation, that has to be designed as a separate, auditable feature; "log in as admin and read DMs" is not supported.
...
<Aside type="note">
  Authorized live events and API responses still contain profile fields in plaintext because clients need to render them. The durable event log is the protected boundary here.
</Aside>
...
Users can delete their own account from account settings or through the public API. Deletion is comprehensive (see [Crypto-shredding](#crypto-shredding) above) and does not leave plaintext message bodies behind. There is no soft-delete or grace period — deletion is immediate and irreversible.
