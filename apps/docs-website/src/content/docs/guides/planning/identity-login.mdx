...
External login providers are configured as repeated `[[auth.providers]]` entries. Supported provider types are `oidc`, `github`, `gitlab`, `google`, `discord`, and `atproto`.
...
<CardGrid>
  ...
  <LinkCard title="Permissions & Roles" href="/guides/planning/permissions/" description="Owner recovery, built-in roles, and permission resolution." />
