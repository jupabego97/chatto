...
Supported provider types are `oidc`, `github`, `gitlab`, `google`, `discord`, and `atproto`.

<EnvVar name="CHATTO_AUTH_PROVIDERS_<index>_<field>" toml="auth.providers">
  Counted environment-variable equivalent of repeated `[[auth.providers]]` tables. Indexes start at `0` and must not have gaps. Supported fields are `ID`, `TYPE`, `LABEL`, `CLIENT_ID`, `CLIENT_SECRET`, `ISSUER_URL`, `SCOPES`, `REQUEST_EMAIL`, `AUTO_PROVISION`, and `PROVIDER_OPTIONS_<KEY>`.
</EnvVar>
...
AT Protocol sign-in is configured through the same provider list and does not use a client ID or client secret:
...
`request_email` defaults to `false` for OAuth/OIDC providers and `true` for AT Protocol. Set `request_email = true` when you want provider-verified email claims. OIDC providers can run with only `openid profile`; Chatto links and signs in accounts by issuer and subject even when no email claim is returned.

`auto_provision` defaults to `false` for OAuth/OIDC providers and `true` for AT Protocol. Enable it only for providers whose users should be allowed to create accounts. Unlinked identities from providers without auto-provisioning can still be linked by existing signed-in users from account settings.
