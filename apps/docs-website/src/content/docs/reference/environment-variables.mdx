...
The only supported provider type is `oidc`.
...
Legacy `CHATTO_AUTH_OIDC_*` variables are not accepted. If any are present, Chatto fails startup with a migration error instead of silently booting without external login providers. See [External Login Providers](/guides/external-login-providers/) for callback URLs, multiple-provider examples, account-linking behavior, and discovery behavior.
