...
- **Wrap form inputs in a real `<form>` element.** Any panel with text inputs and a Save/submit button must use `<form onsubmit={handler}>` with `type="submit"` on the primary button — not `onclick` on a plain button. This is what gives users Enter-to-submit, native validation, browser autofill, and keyboard accessibility. Do `e.preventDefault()` in the handler to suppress native navigation. The settings page (`frontend/src/routes/chat/[serverId]/settings/+page.svelte`) is a reference implementation.
...
- **`PaneHeader` never contains chunky buttons.** The `actions` snippet on `PaneHeader` is reserved for icon-only affordances via `<HeaderIconButton>` (close, bell, leave-room, settings, etc.). Primary actions like "Create Role" / "Save" / "Cancel" must live in the page body — typically inside a `<Panel>` near the top of the body, or alongside their form. For "back to parent route" navigation, use `PaneHeader`'s `backHref` (or `onBack` for slideover-style overlays) — that renders the standard back-arrow, not a button.
- **Use the `link` utility class for inline links, not `text-primary`.** The "primary" design token resolves to a slate/neutral gray in this theme, so `text-primary` links look indistinguishable from muted body text. `link` applies `text-accent hover:underline` (violet — the canonical link color); see `app.css` and the `Foundations/Utilities` Storybook story. The login footer's `text-muted hover:text-primary` pattern is a deliberate exception for subdued small-print links.
- **Don't double-nest `<Panel>`s.** A matrix or list that already renders its own category-level Panels (e.g. `PermissionMatrix`, `SubjectPermissionsMatrix`) should NOT be wrapped in another outer Panel on its page — the nested chrome reads as visual noise and doubles up borders/padding. If the embedded matrix needs introductory prose, put a `<Hint>` above it; if it needs a heading, the matrix's own Panels carry that already.
