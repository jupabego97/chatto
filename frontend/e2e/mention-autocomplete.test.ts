...
test.describe('Mention autocomplete', () => {
  ...
  test.describe('Fuzzy matching', () => {
    test('matches on substring (contains match)', async ({ page, chatPage, roomPage }) => {
      // Create user with login like "foobar1234567890"
      const user = await createAndLoginTestUser(page, { loginPrefix: 'foobar' });
      await chatPage.goto();
      await chatPage.createSpace();
      await chatPage.enterRoom('general');

      // Type @bar which should match "foobar..." (contains "bar")
      await roomPage.messageInput.click();
      await roomPage.messageInput.pressSequentially('@bar');
      await roomPage.messageInput.press('Tab');

      // Should complete to the foobar user
      await expect(roomPage.messageInput).toHaveText(`@${user.login} `);
    });

    test('matches on fuzzy subsequence', async ({ page, chatPage, roomPage }) => {
      // Create user with login like "foobar1234567890"
      const user = await createAndLoginTestUser(page, { loginPrefix: 'foobar' });
      await chatPage.goto();
      await chatPage.createSpace();
      ...
      // Type @fb which should match "foobar..." (f...b subsequence)
      await roomPage.messageInput.click();
      await roomPage.messageInput.pressSequentially('@fb');
      await roomPage.messageInput.press('Tab');
    ...
    test('prefix matches rank higher than fuzzy matches', async ({
      page,
      chatPage,
      roomPage,
      browser,
      ...
      // User 1 creates space with login "foobar..."
      const user1 = await createAndLoginTestUser(page, { loginPrefix: 'foobar' });
      await chatPage.goto();
      const spaceName = await chatPage.createSpace();
      await chatPage.enterRoom('general');

      // User 2 joins with login "fb..." (fb is prefix for user2, fuzzy for user1)
      const context2 = await browser!.newContext({
        baseURL: serverURL,
        viewport: { width: 1280, height: 720 }
      });
      ...
      try {
        const user2 = await createAndLoginTestUser(page2, { loginPrefix: 'fb' });
        ...
        // Type @fb - should match both users:
        // - user2 (fb...) with prefix match (higher score)
        // - user1 (foobar...) with fuzzy match (lower score)
        await roomPage.messageInput.click();
        await roomPage.messageInput.pressSequentially('@fb');
        await roomPage.messageInput.press('Tab');

        // First completion should be the prefix match (user2)
        const inputValue = (await roomPage.messageInput.textContent()) ?? '';
        expect(inputValue).toBe(`@${user2.login} `);

        // Tab again to cycle to the fuzzy match (user1)
        await roomPage.messageInput.press('Tab');
        const inputValue2 = (await roomPage.messageInput.textContent()) ?? '';
        expect(inputValue2).toBe(`@${user1.login} `);
      } finally {
        await context2.close();
      }
    });
