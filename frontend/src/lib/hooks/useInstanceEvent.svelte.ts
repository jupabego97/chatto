...
} from '$lib/instanceEventBus.svelte';
 ...
 * });
 */
export function useInstanceEvent(handler: EventHandler) {
 ...
 * Reads instance ID from Svelte context (set by [[instanceId=hostname]] layout).
 */
export function useActiveInstanceEvent(handler: EventHandler) {
  ...
  $effect(() => {
    ...
    if (!id) return;
    const bus = instanceEventBusManager.getBus(id);
    if (!bus) return;
