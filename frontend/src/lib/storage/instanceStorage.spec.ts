...
describe('instanceStorageKey', () => {
	it('produces a namespaced key', () => {
		expect(instanceStorageKey('chat-example-com', 'lastRooms')).toBe(
			'chatto:i:chat-example-com:lastRooms'
		);
	});

	it('handles different instance IDs', () => {
		expect(instanceStorageKey('localhost', 'lastSpace')).toBe('chatto:i:localhost:lastSpace');
	});

	it('handles compound suffixes', () => {
		expect(instanceStorageKey('my-instance', 'space:abc:collapsed-sections')).toBe(
