...
import { InstanceStateStore } from './store.svelte';
 ...
 * The active instance is determined by the URL (via the [[instanceId=hostname]] layout)
 * and provided to components through Svelte context.
 */
class InstanceRegistry {
	instances = $state<RegisteredInstance[]>(loadInstances());
	#stores = new SvelteMap<string, InstanceStateStore>();
	 ...
	 */
	get originInstance(): RegisteredInstance | undefined {
	 ...
	 */
	isOriginInstance(instanceId: string): boolean {
	...
	probeOrigin(knownInstance = false): void {
		...
		fetch(`${origin}/api/instance`)
			...
			.then((data) => {
				if (!data) return;
				if (this.originInstance) return; // Registered while we were fetching
	...
	removeInstance(id: string): boolean {
		...
		instanceEventBusManager.stopBus(id);
	...
	removeAll(): void {
		for (const instance of [...this.instances]) {
			instanceEventBusManager.stopBus(instance.id);
	 ...
	 */
	getStore(instanceId: string): InstanceStateStore {
		const store = this.#stores.get(instanceId);
		if (!store) {
			throw new Error(
				`No store for instance "${instanceId}". Is it registered? ` +
					`Call instanceRegistry.init() before accessing stores.`
			);
		}
		return store;
	}
	 ...
	 */
	tryGetStore(instanceId: string): InstanceStateStore | undefined {
