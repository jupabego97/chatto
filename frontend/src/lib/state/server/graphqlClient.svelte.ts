...
/** Construct the protobuf wire WebSocket URL matching a GraphQL WebSocket URL. */
export function graphqlWsToWireUrl(wsUrl: string): string {
  const marker = '/api/graphql';
  const index = wsUrl.indexOf(marker);
  if (index === -1) return wsUrl;
  return `${wsUrl.slice(0, index)}/api/wire${wsUrl.slice(index + marker.length)}`;
}

function hostFromAbsoluteUrl(url: string): string {
  return /^[a-z][a-z\d+.-]*:\/\/([^/?#]+)/i.exec(url)?.[1] ?? url;
}
...
export class GraphQLClient {
  ...
  constructor(config: GraphQLClientConfig) {
    ...
    this.wireUrl = graphqlWsToWireUrl(wsUrl);
    this.token = token;
    this.#host = url.startsWith('/')
      ? typeof window !== 'undefined'
        ? window.location.host
        : 'localhost'
      : hostFromAbsoluteUrl(url);
        ...
        ping: (received) => {
          ...
          this.#pongTimeoutId = setTimeout(() => {
            if (this.#activeSocket?.readyState === WebSocket.OPEN) {
              console.log('[ws:%s] Pong timeout (no response in 5s), closing socket', this.#host);
        ...
        mapExchange({
          onResult: (result) => {
            if (
              ...
              console.warn('[auth] GraphQL "not authenticated" error → triggering auth failure', {
