...
import { startWireEventLogger, type WireEventLogger } from './wireEventLogger';
...
function errorDebug(error: unknown) {
  const graphQLErrors = (
    error as { graphQLErrors?: Array<{ message?: string; extensions?: { code?: unknown } }> }
  )?.graphQLErrors;
...
class EventBusManager {
  ...
  #wireLoggers = new Map<string, WireEventLogger>();
  ...
  startBus(serverId: string, gqlClient: GraphQLClient): () => void {
    if (this.#buses.has(serverId)) {
      this.#startWireLogger(serverId, gqlClient);
  ...
  #startWireLogger(serverId: string, gqlClient: GraphQLClient): void {
    if (this.#wireLoggers.has(serverId)) return;
    this.#wireLoggers.set(serverId, startWireEventLogger(serverId, gqlClient));
  }

  /** Stop and remove the event bus for the given server. */
  stopBus(serverId: string): void {
    this.#wireLoggers.get(serverId)?.stop();
    this.#wireLoggers.delete(serverId);
