...
func (r *PermissionResolver) resolveWithGroup(ctx context.Context, userID string, kind RoomKind, roomID, explicitGroupID string, perm Permission) (DecisionKind, error) {
	...
	if r.activeSuspensionDenies(userID, kind, roomID, groupID, perm) {
...
func (r *PermissionResolver) activeSuspensionDenies(userID string, kind RoomKind, roomID, groupID string, perm Permission) bool {
	...
	for _, target := range r.applicableScopeTargets(kind, roomID, groupID, perm) {
		if r.decisionFor(target.scope, target.id, RoleSuspended, parts) == DecisionDeny {
			return true
		}
	}
	return false
...
func (r *PermissionResolver) visitApplicableDecisions(
	...
	subjects := make([]string, 0, len(roles)+2)
	if r.core.UserSuspensions != nil && r.core.UserSuspensions.IsActive(userID, time.Now()) {
		subjects = append(subjects, RoleSuspended)
	}
	subjects = append(subjects, userID)
