...
// TestChattoCore_CreateRoom_DuplicateName_DifferentSpaces was removed in
// PR 7 of the Phase 2 refactor. Per ADR-021 / ADR-029 there is exactly one
// server, rooms are server-wide, and the room-name uniqueness check is
// server-wide. The premise of this test (per-space namespace for room
// names) is gone.
...
// TestRoomMemberships_MultipleSpaces_Isolation was removed in PR 7 of the
// Phase 2 refactor. Per ADR-021 / ADR-029 there is one server and rooms
// are server-wide; per-space membership isolation no longer applies.
...
func TestChattoCore_ListFollowedThreads(t *testing.T) {
	...
	t.Run("returns threads across multiple spaces", func(t *testing.T) {
		t.Skip("Per ADR-021 / ADR-029 (PRs 4 + 7) threads are server-wide; the " +
			"earlier subtests in this function leave state in the unified " +
			"THREADS bucket that this subtest's count assumption ignores. " +
			"The cross-space filtering behavior it tested is gone (one server " +
			"= one set of threads).")
	})
}
