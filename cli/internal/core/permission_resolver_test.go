...
func TestPermissionResolver_HasInstancePermission(t *testing.T) {
	...
	t.Run("returns error for permission that does not apply at instance scope", func(t *testing.T) {
		...
		_, err := core.permissionResolver.HasInstancePermission(ctx, user.Id, PermServerManage)
		if err == nil {
...
func TestPermissionResolver_HasInstancePermission_DenyWins(t *testing.T) {
	...
	t.Run("same-role denial replaces grant", func(t *testing.T) {
		...
		err := core.GrantInstanceRolePermission(ctx, RoleEveryone, PermDMWrite)
		if err != nil {
			t.Fatalf("Failed to grant permission: %v", err)
		}

		// Deny same permission for the same role (replaces the grant)
		err = core.DenyInstanceRolePermission(ctx, RoleEveryone, PermDMWrite)
		if err != nil {
		...
		core.GrantInstanceRolePermission(ctx, RoleEveryone, PermDMWrite)
	})
}
...
func TestPermissionResolver_HasInstancePermission_CustomDenyRole(t *testing.T) {
	...
	err = core.DenyInstancePermission(ctx, "instance-denytest", PermDMWrite)
	if err != nil {
		t.Fatalf("Failed to deny permission: %v", err)
	}
...
func TestPermissionResolver_HasInstancePermission_Hierarchy(t *testing.T) {
	...
	t.Run("higher-ranked role grant beats lower-ranked role denial", func(t *testing.T) {
		...
		err := core.DenyInstanceRolePermission(ctx, RoleEveryone, PermDMView)
		if err != nil {
			t.Fatalf("Failed to deny permission: %v", err)
		}

		// Grant space.create for admin (high rank, position 1)
		err = core.GrantInstanceRolePermission(ctx, RoleAdmin, PermDMView)
		if err != nil {
			t.Fatalf("Failed to grant permission: %v", err)
		}
		...
		// Cleanup
		core.ClearInstanceRolePermission(ctx, RoleEveryone, PermDMView)
		core.ClearInstanceRolePermission(ctx, RoleAdmin, PermDMView)
	})

	t.Run("higher-ranked role denial beats lower-ranked role grant", func(t *testing.T) {
		// Grant space.create for everyone (low rank)
		err := core.GrantInstanceRolePermission(ctx, RoleEveryone, PermDMView)
		if err != nil {
			t.Fatalf("Failed to grant permission: %v", err)
		}

		// Deny space.create for admin (high rank)
		err = core.DenyInstanceRolePermission(ctx, RoleAdmin, PermDMView)
		if err != nil {
			t.Fatalf("Failed to deny permission: %v", err)
		}
...
func TestPermissionResolver_HasSpacePermission(t *testing.T) {
	...
	t.Run("returns true when user has permission via space role", func(t *testing.T) {
		...
		has, err := core.permissionResolver.HasSpacePermission(ctx, user.Id, space.Id, PermServerManage)
		if err != nil {
	...
	t.Run("returns false when user lacks permission at space level", func(t *testing.T) {
		...
		has, err := core.permissionResolver.HasSpacePermission(ctx, otherUser.Id, space.Id, PermServerManage)
		if err != nil {
			t.Fatalf("HasSpacePermission() error = %v", err)
		}
		if has {
...
func TestPermissionResolver_HasSpacePermission_InstanceFallback(t *testing.T) {
	...
	// Per ADR-028 the special-case exception that let non-members evaluate
	// space.join / space.list at space scope is removed alongside the dropped
	// permissions themselves; non-members now uniformly get false for any
	// space-scoped permission via HasSpacePermission.
}
...
func TestPermissionResolver_HasSpacePermission_DenyWins(t *testing.T) {
	...
	t.Run("deny-wins at space level", func(t *testing.T) {
		...
		err := core.GrantSpaceRolePermission(ctx, space.Id, RoleEveryone, PermMessagePost)
		if err != nil {
			t.Fatalf("Failed to grant permission: %v", err)
		}

		// Deny to admin role (user is admin)
		err = core.DenySpaceRolePermission(ctx, space.Id, RoleOwner, PermMessagePost)
		if err != nil {
			t.Fatalf("Failed to deny permission: %v", err)
		}
...
func TestPermissionResolver_HasSpacePermission_InstanceRoleOverride(t *testing.T) {
	...
	t.Run("space can override instance role permissions", func(t *testing.T) {
		...
		err := core.GrantSpaceRolePermission(ctx, space.Id, RoleEveryone, PermRoomManage)
		if err != nil {
			t.Fatalf("Failed to grant permission: %v", err)
		}
...
func TestPermissionResolver_HasSpacePermission_DMSpace(t *testing.T) {
	...
	t.Run("DM space denies space.manage", func(t *testing.T) {
		has, err := core.permissionResolver.HasSpacePermission(ctx, user.Id, DMSpaceID, PermServerManage)
		if err != nil {
			t.Fatalf("HasSpacePermission() error = %v", err)
		}
		if has {
...
func TestPermissionResolver_HasRoomPermission(t *testing.T) {
	...
	t.Run("falls back to space level", func(t *testing.T) {
		...
		err := core.GrantSpaceRolePermission(ctx, space.Id, RoleOwner, PermRoomManage)
		if err != nil {
...
func TestPermissionResolver_HasRoomPermission_RoomGrantCannotOverrideSpaceDenial(t *testing.T) {
	...
	// Deny at space level
	core.DenySpaceRolePermission(ctx, space.Id, RoleEveryone, PermMessagePost)
...
func TestPermissionResolver_DenyAlwaysWins(t *testing.T) {
	...
	t.Run("space deny blocks room grant", func(t *testing.T) {
		// Deny at space level
		err := core.DenySpaceRolePermission(ctx, space.Id, RoleEveryone, PermMessageReact)
		if err != nil {
	...
	t.Run("instance deny blocks room grant", func(t *testing.T) {
		// Deny at instance level for instance-everyone
		err := core.DenyInstanceRolePermission(ctx, RoleEveryone, PermMessagePost)
		if err != nil {
			t.Fatalf("Failed to deny instance permission: %v", err)
		}
	...
	t.Run("space deny blocks instance grant", func(t *testing.T) {
		...
		err := core.GrantInstanceRolePermission(ctx, RoleEveryone, PermRoomCreate)
		if err != nil {
			t.Fatalf("Failed to grant instance permission: %v", err)
		}

		// Deny at space level via space config for instance-everyone
		err = core.DenySpaceRolePermission(ctx, space.Id, RoleEveryone, PermRoomCreate)
		if err != nil {
			t.Fatalf("Failed to deny space permission: %v", err)
		}
		...
		has, err := core.permissionResolver.HasSpacePermission(ctx, newUser.Id, space.Id, PermRoomCreate)
		if err != nil {
			t.Fatalf("HasSpacePermission() error = %v", err)
		}
		if has {
...
func TestPermissionResolver_InstanceAuthority(t *testing.T) {
	...
	t.Run("instance grant applies for space member", func(t *testing.T) {
		...
		has, err := core.permissionResolver.HasSpacePermission(ctx, member.Id, space.Id, PermRoomCreate)
		if err != nil {
			t.Fatalf("HasSpacePermission() error = %v", err)
		}
		if !has {
