...
func TestPermissionResolver_ActiveSuspensionDeniesBeforeNormalGrants(t *testing.T) {
	...
	if err := core.GrantUserPermission(ctx, SystemActorID, target.Id, PermAdminUsersView); err != nil {
		t.Fatalf("GrantUserPermission: %v", err)
	}

	has, err := core.HasServerPermission(ctx, target.Id, PermAdminUsersView)
	if err != nil {
		t.Fatalf("baseline admin.view-users: %v", err)
	}
	if !has {
		t.Fatal("baseline: owner with user grant should have admin.view-users")
	}
	...
	has, err = core.HasServerPermission(ctx, target.Id, PermAdminUsersView)
	if err != nil {
		t.Fatalf("suspended admin.view-users: %v", err)
	}
	if has {
