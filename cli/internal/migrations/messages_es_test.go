...
func TestMigrateMessagesToES_ResumesAfterBodySideEventLanded(t *testing.T) {
	...
	if _, err := rig.publisher.AppendAt(rig.ctx, events.RoomAggregate("R1").Subject(events.EventMessageBody), &corev1.Event{
		Id:        "message_body.M1",
		ActorId:   "U1",
		CreatedAt: timestamppb.Now(),
		Event: &corev1.Event_MessageBody{
			MessageBody: &corev1.MessageBodyEvent{
				...
				Body: &corev1.MessageBody{
					...
					BodyEventId:     "message_body.M1",
				},
			},
		},
	}, 0); err != nil {
		t.Fatalf("seed body side event: %v", err)
	}
	...
	if info.State.Msgs != 2 {
		t.Errorf("EVT msg count = %d, want 2 (preexisting body + committed message)", info.State.Msgs)
	}
	checkImportedMessage(t, rig.readEvent(t, 2), "M1", "U1")
}

func TestMigrateMessagesToES_ResumesAfterThreadSideEventLanded(t *testing.T) {
	rig := setupMessagesRig(t)
	rig.publishLegacy(t, "server.room.channel.R1.msg.ROOT.replies.REPLY1", &corev1.Event{
		Id:        "REPLY1",
		ActorId:   "U2",
	...
	if _, err := rig.publisher.AppendAt(rig.ctx, events.RoomAggregate("R1").Subject(events.EventThreadCreated), &corev1.Event{
		Id:        migratedThreadCreatedEventID("R1", "ROOT"),
		ActorId:   "U2",
		CreatedAt: timestamppb.Now(),
		...
		t.Fatalf("seed thread side event: %v", err)
	}
	...
	if info.State.Msgs != 2 {
		t.Errorf("EVT msg count = %d, want 2 (preexisting thread + committed reply)", info.State.Msgs)
	}
	checkImportedMessage(t, rig.readEvent(t, 2), "REPLY1", "U2")
}
...
func TestMigrateMessagesToES_ImportsLargeRoom(t *testing.T) {
	rig := setupMessagesRig(t)
...
func TestMigrateMessagesToES_ImportsLargeRoomWithBodiesNonAtomically(t *testing.T) {
	rig := setupMessagesRig(t)
	const total = messageMigrationBatchSize

	for i := 0; i < total; i++ {
		eventID := fmt.Sprintf("M%03d", i)
		bodyKey := "U1." + eventID + "-BODY"
		rig.putBody(t, bodyKey, &corev1.MessageBody{
			AuthorId:      "U1",
		...
		rig.publishLegacy(t, "server.room.channel.R1.msg."+eventID, &corev1.Event{
			...
			CreatedAt: timestamppb.New(time.Unix(int64(i), 0)),
			Event: &corev1.Event_MessagePosted{
				MessagePosted: legacyPosted("R1", bodyKey),
			},
		})
	}
	...
	if info.State.Msgs != total*2 {
		t.Errorf("EVT msg count = %d, want %d", info.State.Msgs, total*2)
	}
}
