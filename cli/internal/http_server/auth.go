...
func (s *HTTPServer) setupAuthRoutes() {
	...
	// Bootstrap endpoint - creates first admin and initial space
	// No authentication required (this creates the first user)
	auth.POST("bootstrap", func(c *gin.Context) {
		var req struct {
			Login            string  `json:"login" binding:"required"`
		...
		if req.SpaceName != nil {
			input.SpaceName = *req.SpaceName
		}
		if req.SpaceDescription != nil {
			input.SpaceDescription = *req.SpaceDescription
		}

		result, err := s.core.Bootstrap(ctx, input)
		if err != nil {
			if errors.Is(err, core.ErrAlreadyBootstrapped) {
				c.JSON(http.StatusConflict, gin.H{"error": "this instance has already been set up"})
				return
			}
		...
		if err := session.Save(); err != nil {
			log.Error("Failed to save session after bootstrap", "error", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create session"})
			return
		}

		log.Info("Instance bootstrapped and admin logged in", "userId", result.User.Id)
		...
		if token, err := s.core.CreateAuthToken(ctx, result.User.Id); err == nil {
			response["token"] = token
		} else {
			log.Warn("Failed to create auth token on bootstrap", "userId", result.User.Id, "error", err)
		}
