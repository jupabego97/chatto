...
func TestGraphQL_Query_Room_RequiresMembership(t *testing.T) {
	...
	space, err := env.core.CreateSpace(env.ctx, userID, "Room Test Space", "")
	if err != nil {
		t.Fatalf("Failed to create space: %v", err)
	}
...
func TestGraphQL_Mutation_CreateSpace_Unauthenticated(t *testing.T) {
	env := setupGraphQLTestServer(t)

	resp := env.doGraphQL(t, `mutation($input: CreateSpaceInput!) {
	...
	if len(resp.Errors) == 0 {
		t.Error("Expected authentication error for unauthenticated mutation")
	}
	...
	if !foundAuthError {
		t.Errorf("Expected authentication error, got: %v", resp.Errors)
	}
}

func TestGraphQL_Mutation_CreateSpace_Authenticated(t *testing.T) {
	env := setupGraphQLTestServer(t)

	// Create and login verified user (verified users can create spaces)
	env.createVerifiedTestUser(t, "createspace", "password123")
	env.login(t, "createspace", "password123")
	...
	if data.CreateSpace.Name != "My New Space" {
		t.Errorf("Expected name 'My New Space', got %s", data.CreateSpace.Name)
	}
	if data.CreateSpace.ID == "" {
...
func TestGraphQL_CryptoShredding_MessageBodyBecomesNull(t *testing.T) {
	...
	space, err := env.core.CreateSpace(env.ctx, userID, "Test Space", "")
	if err != nil {
		t.Fatalf("Failed to create space: %v", err)
	}

	room, err := env.core.CreateRoom(env.ctx, userID, space.Id, "general", "")
	if err != nil {
		t.Fatalf("Failed to create room: %v", err)
	}

	// Join room (actor, space, user, room)
	_, err = env.core.JoinRoom(env.ctx, userID, space.Id, userID, room.Id)
	if err != nil {
