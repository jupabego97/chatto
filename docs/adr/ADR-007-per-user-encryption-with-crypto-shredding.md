...
- **Algorithm**: New message bodies use a compact versioned envelope: XChaCha20-Poly1305 encrypts the body with the author's active message-body DEK epoch. Each DEK is generated by Chatto, wrapped through the KMS boundary, stored as a protobuf `StoredUserDEK` wrapped-key record in `ENCRYPTION_KEYS` under an opaque content-key ref, and recorded for audit as a durable `UserDEKGeneratedEvent` containing only refs and wrapping metadata. Legacy bodies encrypted directly with the per-user ChaCha20-Poly1305 key remain readable.
...
- **Per-user keys**: Each user has their own KEK. New KEKs are stored behind opaque KMS key refs in the dedicated `ENCRYPTION_KEYS` KV bucket; app-owned wrapped DEK records live in the same bucket under opaque content-key refs. DEK events record the content-key ref and the key ref used to wrap it.
...
- **Erasure = key deletion + durable shred event**: When a user requests deletion, the app-owned DEK records and KMS wrapping-key refs recorded on their DEK events are shredded and a `UserKeyShreddedEvent` is appended to the user aggregate. All their encrypted message bodies and durable PII payloads become permanently unreadable, and projections treat the shred event as the authoritative tombstone signal before attempting decrypts.
...
- **KMS boundary**: KEK creation, content-key wrapping/unwrapping, and KEK shredding (`createKey`, `wrapContentKey`, `unwrapContentKey`, `shredKey`) go through the dedicated `internal/kms` interface keyed by opaque KMS refs rather than Chatto user IDs. DEK record create/load/shred is application-owned `ENCRYPTION_KEYS` storage, keeping wrapped DEKs available locally while still making individual DEK refs crypto-shreddable. The default KMS implementation is in-process and backed by `ENCRYPTION_KEYS`; it can be extracted to a standalone service for high-security deployments. Legacy direct-key body decrypt is the only remaining raw-KEK compatibility path.

## Consequences

- **Fast, reliable erasure**: Shredding the user's DEK refs or wrapping-key refs renders their encrypted message bodies and durable PII unreadable. The durable shred event lets room timeline and thread projections immediately tombstone already-projected or replayed messages authored by that user.
...
- **Per-message overhead**: Legacy bodies carry one nonce and Poly1305 tag. V2 message bodies carry a body nonce, Poly1305 tag, and compact `content_key_epoch` payload field referencing the author's message-body DEK. Wrapped DEK material is stored once per user, purpose, and epoch in app-owned `ENCRYPTION_KEYS` storage; EVT keeps only the auditable content-key ref and wrapping-key ref.
...
- **Future extensibility**: The KMS interface, wrapping-key refs, and wrapping metadata on DEK events can be adapted to external key management (HashiCorp Vault, AWS KMS, HSM) without moving wrapped DEK storage into the external KMS API.
