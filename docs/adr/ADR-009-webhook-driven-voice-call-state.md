...
- **Memory-backed state is intentional**: `MEMORY_CACHE` uses `MemoryStorage` and is excluded from backups. If JetStream restarts, call state is lost — but participants will reconnect to LiveKit, triggering new join webhooks that repopulate the state. Persistence would be wasted storage. The retired `CALL_STATE` bucket is copied into `MEMORY_CACHE` on boot when present so active calls survive the storage rename during upgrade.
