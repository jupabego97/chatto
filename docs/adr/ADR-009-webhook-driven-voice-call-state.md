...
- On startup and periodically, the elected call reconciler compares projected state to LiveKit's current room/participant state and appends `RECONCILIATION` facts for mismatches. Only the replica holding the short-lived `MEMORY_CACHE` lease `lease.livekit_reconciler` runs this periodic reconciliation loop. If LiveKit listing fails before the destructive threshold, the leader records the failed pass in `MEMORY_CACHE` key `livekit.reconciliation.list_failures` and keeps running on the normal reconciliation ticker. Any successful elected reconcile pass deletes that shared failure counter, so failures are consecutive at the elected-reconciler level rather than tied to one process. If the elected reconciler cannot list rooms/participants for three consecutive reconciliation cycles, Chatto treats the media service as unavailable and appends `RECONCILIATION` leaves/ends for all projected active calls so dead calls do not block new sessions. Call transition appends use the call projection's per-room applied sequence as the OCC token against `evt.room.{roomId}.>`; on conflict, the service retries from a fresh projection snapshot and skips the append if another writer already applied the transition. First joins append `call_started + call_joined` atomically; final leaves append `call_left + call_ended` atomically.
- Call lifecycle and participant EVT facts are delivered through the durable live EVT subscription path, but they are hidden from normal visible room timelines.
...
- **Reconciliation**: A process restart no longer loses the local active participant snapshot permanently; the elected reconciler queries LiveKit and appends correction facts for rooms/participants that differ from the projection. Missing LiveKit rooms and observed empty rooms still end calls immediately after a successful listing because they are authoritative LiveKit observations. LiveKit listing failures are tolerated until three consecutive elected reconciliation failures, counted in shared `MEMORY_CACHE` state and reset by any successful elected pass; pre-threshold failures are logged and retried on the normal reconciliation ticker. The `MEMORY_CACHE` lease keeps periodic reconciliation single-leader across replicas, while the room aggregate OCC boundary still protects against duplicate transition facts from webhooks, user intent, or failover races.
...
- **E2EE compatibility**: New clients always enable LiveKit E2EE using a per-call shared key returned with `voiceCallToken`, which also returns the active call ID so connected clients can ignore stale leave/end facts from previous calls in the same room. Observer/sidebar call participant reads include the same call ID for the same stale-event guard. Older clients without E2EE will not decode encrypted media.
