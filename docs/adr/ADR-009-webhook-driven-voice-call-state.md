...
- On startup and periodically, the call-state service compares projected state to LiveKit's current room/participant state and appends `RECONCILIATION` facts for mismatches. If LiveKit cannot list rooms/participants, Chatto treats the media service as unavailable and immediately appends `RECONCILIATION` leaves/ends for all projected active calls so dead calls do not block new sessions. Call transition appends use the call projection's per-room applied sequence as the OCC token against `evt.room.{roomId}.>`; on conflict, the service retries from a fresh projection snapshot and skips the append if another replica already applied the transition. First joins append `call_started + call_joined` atomically; final leaves append `call_left + call_ended` atomically.
...
- **Reconciliation**: A process restart no longer loses the local active participant snapshot permanently; the service queries LiveKit and appends correction facts for rooms/participants that differ from the projection. If LiveKit listing fails, projected active calls are ended immediately with reconciliation facts. The room aggregate OCC boundary lets multiple replicas reconcile without a leader lease while avoiding duplicate transition facts after OCC conflicts.
