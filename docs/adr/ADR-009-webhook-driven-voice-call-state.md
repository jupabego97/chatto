...
- `participant_joined` adds the participant to a memory-backed `CALL_STATE` KV bucket
...
- **Memory-backed state is intentional**: The `CALL_STATE` KV bucket uses `MemoryStorage`. If the Chatto server restarts, call state is lost — but participants will reconnect to LiveKit, triggering new join webhooks that repopulate the state. Persistence would be wasted storage.
