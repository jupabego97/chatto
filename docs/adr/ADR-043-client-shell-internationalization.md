...
Message keys should be semantic and stable rather than English-as-key. Catalog files should use nested JSON grouped by feature or UI surface, such as `settings.preferences.title`, `auth.login.submit`, or `room.event.user_joined`. The message-format plugin flattens nested JSON internally, and Paraglide exposes nested paths as quoted exports used with bracket notation, such as `m["settings.preferences.title"]()`. If Paraglide later provides a stronger Svelte-specific namespacing convention, Chatto may adopt it, but new code must still keep keys stable across wording changes.
