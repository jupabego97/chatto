...
2. **Rework API and frontend UX around the Server concept.** Anything user-facing or API-facing that currently splits instance from space gets reframed as a single layer routed through the primary space: public API additions/aliases, frontend stores, navigation, settings pages, admin views, "this deployment" surfaces. Underlying storage and the legacy multi-space code paths stay intact behind the new surface.
