...
- **Projection consumer sharing is a follow-on runtime optimization.** The initial implementation used one NATS `OrderedConsumer` per projection. With many projections live, [ADR-038](ADR-038-shared-projection-consumers.md) updates that runtime shape to share one consumer per canonical subject filter set while keeping `WaitForSeq`, lag, and failure state per projection.
...
- Projection consumer sharing — now covered by [ADR-038](ADR-038-shared-projection-consumers.md).
