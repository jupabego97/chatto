...
- **Projection consumer multiplexing is handled above the stream.** A projection that needs events for all rooms declares a wildcard filter (`evt.room.>`). [ADR-038](ADR-038-shared-projection-consumers.md) lets projections with the same canonical filter set share one consumer without changing the single-stream layout.
