...
- **Client live WebSocket PoC**: Experimental browser-facing protobuf WebSocket transport for live updates plus multiplexed request/response message history reads. `GET /api/server` advertises `live` discovery metadata, authenticated clients mint a short-lived ticket through `POST /api/live-token`, and `GET /api/live` starts authorized delivery immediately. The websocket handler calls `StreamMyEvents` against Core for push delivery and projection-backed Core read methods for history requests; raw `live.evt.>` and `live.sync.>` subjects remain internal. EVT-derived facts are hydrated into client-facing `corev1.LiveEvent` read-model payloads before they leave the server.
- **Metrics & Diagnostics**: Optional Prometheus-compatible per-process metrics run on a separate internal HTTP listener configured by `[metrics]`. The endpoint exposes Go/process collectors plus Chatto readiness, GraphQL WebSocket counts, client-live WebSocket counts/request totals/request duration sums/protocol errors, `myEvents` stream counters, NATS client counters, projection health/lag gauges, and final projection startup duration/message-count gauges once initial replay completes. Go pprof debug endpoints are available on the same internal listener under `/debug/pprof/` only when `[metrics].pprof` / `CHATTO_METRICS_PPROF` is enabled. `[diagnostics].startup_cpu_profile` / `CHATTO_DIAGNOSTICS_STARTUP_CPU_PROFILE` writes a process-startup CPU profile through core boot for local replay benchmarking and operator troubleshooting.
- **Web Client**: SvelteKit-based SPA that gets compiled and embedded into the Go binary. Talks to GraphQL over HTTP for most current surfaces; the prototyped message live/history path uses the client-live protobuf WebSocket.
...
Key files: [`cli/internal/core/core.go`](../cli/internal/core/core.go), [`cli/internal/events/subjects.go`](../cli/internal/events/subjects.go), [`proto/chatto/core/v1/event.proto`](../proto/chatto/core/v1/event.proto), [`cli/internal/core/subjects/subjects.go`](../cli/internal/core/subjects/subjects.go), [`cli/internal/http_server/live.go`](../cli/internal/http_server/live.go), [`proto/chatto/core/v1/client_live.proto`](../proto/chatto/core/v1/client_live.proto)
...
| HTTP WS      | `/api/live`                 | Protobuf | No     | Experimental browser-facing live event stream; frames are `corev1.ClientLiveServerFrame` / `corev1.ClientLiveClientFrame` |
...
`MyEventsService` owns the `ChattoCore.StreamMyEvents` facade and subscribes to `live.sync.>` and `live.evt.>`. For deliverable raw EVT room and asset messages, it reads the republished `Nats-Sequence` header, waits for the local projections needed by authorization and live view hydration, and filters by the subscribing user. Asset lifecycle events resolve their room authorization through `AssetProjection`, using the room scope on `AssetCreatedEvent` and inherited parent scope for derivatives. Transient `LiveEvent` messages are adapted at this boundary into the same authorized stream. The stream is live-only; missed state is recovered by projected reads. The bundled web client consumes this stream through the client-live protobuf WebSocket; for EVT-derived room timeline events, the WebSocket edge sends viewer-shaped `LiveEvent` payloads such as `LiveRoomEvent`, `LiveMessageReactionsUpdatedEvent`, and `LiveMessageAttachmentsUpdatedEvent` instead of raw durable `Event` protobufs. The GraphQL `myEvents` subscription still exists as API surface, but the bundled web client does not use it in this branch. There is no per-connection JetStream consumer and no public subscription replay cursor.
...
| Live Sync                    | `corev1.LiveEvent` | Transient  | Direct NATS Core pubsub on `live.sync.>` for transient UI sync signals. `StreamMyEvents` authorizes these messages for GraphQL `myEvents` and the client-live WebSocket; they are never projection input. |

The republished `live.evt.{aggregateType}.{aggregateId}.{eventType}` subject is an internal server-side feed; `StreamMyEvents` waits for projections and authorization before delivering anything to GraphQL `myEvents` or the client-live WebSocket.
...
The `StreamMyEvents` core stream, used by GraphQL `myEvents` and the client-live WebSocket, combines:
...
- Live-only delivery. Missed state after reconnect is recovered from projected reads: server-scoped stores refetch their current projections after event-bus gaps, and the visible room/thread refetches its current message window through the client-live request/response protocol when available. Transient sync and presence signals remain live-only.
...
The experimental client live WebSocket transport reuses this same `StreamMyEvents` boundary instead of exposing `live.sync.>` or `live.evt.>` directly. `GET /api/server` includes `live` discovery metadata with the WebSocket URL, token URL `/api/live-token`, and protocol ID `chatto.client-live-protobuf.v1`. A browser client authenticates the token request with its Chatto bearer token for cross-origin servers or its existing cookie session for the same-origin server; the server returns a short-lived HMAC-signed ticket bound to the user and request origin. The client then opens `GET /api/live?ticket=...` cross-origin when allowed by `[webserver].allowed_origins`.

Once upgraded, the server immediately starts live delivery; the client does not send a nested subscribe request. WebSocket frames are binary protobuf messages defined in [`proto/chatto/core/v1/client_live.proto`](../proto/chatto/core/v1/client_live.proto). Server frames wrap authorized `corev1.LiveEvent`, heartbeat, ping, response, or error payloads and include both a per-connection delivery sequence and the underlying stream sequence when available. Durable `corev1.Event` facts remain an internal feed and the raw server-frame event field is deprecated: before delivery, the WebSocket session hydrates supported message posts, edits, reactions, attachment/video-processing updates, retractions, room lifecycle facts, and call facts into client-facing `LiveEvent` payloads defined in [`proto/chatto/core/v1/live_events.proto`](../proto/chatto/core/v1/live_events.proto). `LiveRoomEvent` carries top-level `room_id` for room-scoped client routing, including asset deletion events whose durable payload only contains `asset_id`. Client frames support hello, ack, pong, and typed requests. The current request set includes room latest/before/after pages, around-message room pages, single room-event lookup, thread reply pages, and around-reply thread pages; responses carry hydrated `LiveRoomEvent` items plus stream-sequence cursors. Request handling is bounded per connection and emits low-cardinality metrics for request type, outcome, duration sums, and protocol error codes.
...
- `MessagePostedEvent.channelEchoEventId` is a derived field backed by `RoomTimelineProjection`'s echo-link index; it is not stored in the durable protobuf payload. The client-live WebSocket includes it in hydrated `LiveMessagePostedEvent` responses.
- Initial loads and cursor pagination walk the visible room timeline so thread replies, edits, retractions, reactions, asset-processing facts, and directly hidden echoes do not count as separate room timeline rows. In the bundled web client PoC, latest/before/after/around room history and single-event preview loads use client-live protobuf requests over the same WebSocket as live push when the event bus has an active requester; the GraphQL fields remain available as API surface and fallback for un-wired contexts.
- Reconnect catch-up in the web client refreshes the currently viewed room window from `RoomTimelineProjection` after browser wake, client-live stream end, or heartbeat-stall catch-up notifications. When the user is at the bottom it fetches the latest page; when scrolled up it uses an around-event page for the visible anchor event and preserves scroll by event ID. Normal LiveEvent delivery for the prototyped message timeline path is directly renderable: posts, edits, reaction summaries, attachment/video-processing updates, and historical page items patch local state without a follow-up GraphQL query. Server-scoped stores still refetch projected state after event-bus reconnect/resubscribe gaps so notifications, unread/sidebar state, room layout, server profile/settings, and active-call indicators do not depend on replayed live events. `Subscription.myEvents` and the client-live stream are live-only and do not expose a replay cursor.
- Around-event requests use the same visible room timeline to center jump-to-message windows on the target's visible position.
- Thread panes read the root message from `RoomTimelineProjection` and cursor-paginated replies from `ThreadProjection`. In the bundled web client PoC, initial thread loads, older reply pages, and anchored thread refreshes use client-live protobuf requests with hydrated `LiveRoomEvent` rows.
