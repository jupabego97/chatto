...
- **GraphQL**: Client-facing legacy API for remaining operations (auth, management, messaging) and the legacy realtime subscription surface. Subscriptions over WebSocket can still deliver real-time updates through `myEvents`, but the bundled web client now uses `/api/realtime` for live delivery. Fields require authentication by default unless marked public in the schema; resolvers call Core methods directly and enforce operation-specific authorization before each call.
...
The GraphQL API is the primary client-facing interface for Chatto. It provides queries, mutations, and a single unified subscription over HTTP and WebSocket connections. Fields require authentication by default unless explicitly marked public in the schema. Authentication accepts opaque bearer tokens first and falls back to cookie sessions when no bearer token is present. Authentication failures keep the stable `authentication required` message and include `extensions.code = "UNAUTHENTICATED"` so multi-server clients can invalidate only the affected server credential. User registration, login, password reset, email verification, and OIDC provider login flows are REST endpoints (under `/auth/...`) rather than GraphQL mutations; successful password login, registration, and confirmed OIDC account creation/linking issue both a cookie session and a bearer token. Public server metadata includes `Server.authProviders`, a list of configured OIDC login providers with IDs, types, labels, and login URLs.
...
| `pending_oidc.{hmac}` | Pending first-time OIDC identity JSON created after an unlinked provider callback. Link-current confirmations may carry the initiating Chatto user ID so generic pending tokens cannot attach identities to arbitrary logged-in accounts. Uses per-key 15-minute TTL and is deleted when the user confirms account creation/linking or cancels. |
