...
Key files: [`cli/internal/core/core.go`](../cli/internal/core/core.go), [`cli/internal/events/`](../cli/internal/events/), [`cli/internal/runtimeunit/`](../cli/internal/runtimeunit/), [`cli/internal/connectapi/`](../cli/internal/connectapi/), [`cli/internal/managementapi/`](../cli/internal/managementapi/), [`cli/internal/managementserver/`](../cli/internal/managementserver/), [`cli/internal/http_server/connect.go`](../cli/internal/http_server/connect.go), [`cli/internal/http_server/metrics.go`](../cli/internal/http_server/metrics.go), [`cli/internal/exporter/`](../cli/internal/exporter/), [`proto/chatto/core/v1/`](../proto/chatto/core/v1/), [`proto/chatto/api/v1/`](../proto/chatto/api/v1/), [`proto/chatto/management/v1/`](../proto/chatto/management/v1/)
...
- **Management API**: Private operator-only ConnectRPC management plane mounted on a Unix socket configured by `[management]` (default `.chatto/admin.sock`, `socket_mode = "0600"`). The socket parent directory must be owned by the server user and private by default; shared local operator access requires explicit `socket_mode = "0660"` plus `socket_group` and a non-world-accessible, non-group-writable parent directory with that group. Management protos live in `proto/chatto/management/v1/` and generate Go-only server/client bindings. `chatto user create`, `chatto user update`, and `chatto user password` call this socket so user administration goes through the running `ChattoCore` process without exposing administrative RPCs on the public web/API listener and without direct NATS writes. The first management service is `chatto.management.v1.UserAdminService`; it delegates to existing core user methods and uses the synthetic actor ID `system:operator`.
...
Public API protobufs are intentionally separate from persisted `corev1` EVT protobufs. `ServerInfoState` uses `ServerService.GetServer` for public profile metadata. `MessageService.PostMessage` is the message-write Connect path for text/link-preview posts and for non-browser clients that already have room-scoped attachment asset IDs; browser file uploads still use the GraphQL upload-capable mutation. Both transports delegate to the core `MessageService` so room/thread authorization, large-mention confirmation, echo-to-channel checks, durable `MessageBodyEvent`/`MessagePostedEvent` writes, and post-send read-marker side effects do not drift. `UserStatusService.SetCustomStatus` and `ClearCustomStatus` are authenticated self-service profile mutations that delegate to the core user service, which writes durable user aggregate EVT facts. `NotificationPreferencesService.SetRoomNotificationLevel` is the authenticated/authorized preference mutation PoC: Connect requests reuse the same bearer-token-then-cookie auth injection as GraphQL, then delegate to the core `NotificationPreferencesService`, which enforces channel-room membership before writing the room notification preference. The GraphQL room notification mutation uses the same core service so transport-specific authorization does not drift. `RoomTimelineService.GetRoomEvents`, `GetRoomEventsAround`, `GetThreadEvents`, and `GetThreadEventsAround` are the read-path PoC: they require room membership, read projection-backed room and thread timelines, and return renderable protobuf timeline rows with hydrated users, message bodies, reactions, thread metadata, link previews, and attachment URLs so browser clients avoid resolver-style N+1 fetching. `ReadStateService` and `ThreadService` carry timeline-adjacent write paths: marking rooms/threads read and following/unfollowing threads without a GraphQL fallback. Realtime delivery remains on the existing GraphQL/WebSocket API until later migration work.
