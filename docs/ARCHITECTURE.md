...
| ------------------ | -------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Room directory     | Room Directory       | `evt.room.>`                                               | `RoomCatalogProjection`, `RoomMembershipProjection`, `RoomBanProjection`; room/member queries, room name/alias resolution, room-ID-aware name claims, and room authorization |
...
| `roomByName(name)`                 | Resolve a channel room by current name or replay-derived historical name. Authorization matches `room(roomId)` (membership required). URL consumers decide whether to call this field or the existing `room(roomId)` field; the web client uses `room(roomId)` only for `R...` segments because room IDs always start with capital `R`. Channel-room URLs canonicalize to the room's current `name`; DM URLs remain ID-based because DM rooms have empty names. Current room IDs are reserved by room name claims so channel names cannot collide with ID URLs. |
