...
| ------------------ | -------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Room directory     | Room Directory       | `evt.room.>`                                               | `RoomCatalogProjection`, `RoomMembershipProjection`, `RoomBanProjection`; room/member queries, room name/alias resolution, room name claims, and room authorization |
...
| `roomByName(name)`                 | Resolve a channel room by current name or replay-derived historical name. Authorization matches `room(roomId)` (membership required). The web client uses legacy `/chat/{server}/{roomId}` routes for ID links and `/chat/{server}/r/{roomName}` routes for named channel links. Channel-room URLs canonicalize to `/r/{currentName}`; DM URLs remain ID-based because DM rooms have empty names. |
