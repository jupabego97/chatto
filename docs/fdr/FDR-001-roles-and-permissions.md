...
- The system roles are `owner`, `admin`, `moderator`, `everyone`, plus the protected virtual `suspended` role rendered as `@suspended`. Role position controls ordering/display and legacy event compatibility; it is not a general authorization rank.
...
- Suspension uses RBAC-shaped enforcement: explicit denies from active `@suspended` are checked before ordinary user-level grants, role grants, and owner/admin defaults, so stale ad-hoc grants cannot bypass suspension.
...
### 9. Permission-decision events carry typed scope and subject
...
- `user.suspend` — suspend or unsuspend users.
