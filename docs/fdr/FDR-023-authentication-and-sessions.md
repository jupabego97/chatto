...
Chatto authenticates users via two parallel mechanisms: opaque bearer tokens for API clients and HTTP-only cookie sessions for same-origin compatibility. The bundled SPA stores bearer tokens for registered servers, including the origin after direct password login or registration, while cookie sessions remain for non-GraphQL auth endpoints and same-origin auth flows. Login flows include classic password login, configured OIDC providers, and a bootstrap path for first-boot operator setup.

## Behavior

- **Login** — users sign in with login + password on a `/login` page. The page is also used for redirect-after-signup.
- **External provider login** — operators configure repeated OIDC-only `[[auth.providers]]` entries or equivalent counted `CHATTO_AUTH_PROVIDERS_<index>_<field>` environment variables. The login page renders buttons from `Server.authProviders`, and provider flows use `/auth/providers/{providerID}` plus `/auth/providers/{providerID}/callback`. When exactly one OIDC provider is configured, `/auth/oidc` and `/auth/oidc/callback` remain registered as migration aliases for older IdP callback registrations.
- **External identity matching** — OIDC identities are linked through durable `EVT` user facts and matched by verified issuer URL plus subject. Email claims are optional profile data and are not used as the primary account match key. If a provider callback returns an identity that is not linked to a Chatto account, Chatto stores a short-lived pending OIDC identity in `RUNTIME_STATE` and redirects to a confirmation page. From there, the user can explicitly create a new passwordless Chatto account, link the identity to an existing local account by proving the password, link it to the current logged-in account only when the pending token is bound to that same initiating Chatto user, or cancel.
...
- **GraphQL auth failures** — protected fields return the stable `authentication required` error message with `extensions.code = "UNAUTHENTICATED"`. The bundled multi-server client prefers the structured code and keeps the message fallback so newer clients can still talk to older servers.
...
**Decision:** Same-origin cookie-session writes require a CSRF proof unless they target GraphQL, public auth/bootstrap endpoints, test endpoints, webhooks, or OAuth token exchange. The server mirrors a readable `chatto_csrf` cookie for the SPA and accepts `X-CSRF-Token` only when it matches that cookie and verifies as a server-signed token for the current authenticated user generation. GraphQL requests are treated as API traffic instead: the SPA sends `X-REQUEST-TYPE: GraphQL`, a non-simple request marker that cross-site forms cannot send, and does not send the CSRF token. Bearer-only requests skip the CSRF check.
**Why:** Cookie sessions are ambient browser credentials, so another site can try to submit state-changing requests with the user's cookie attached. Browser form CSRF is a poor fit for the GraphQL API surface, and storing CSRF state inside the signed browser session cookie made concurrent cookie refreshes brittle. Signed stateless tokens keep CSRF independent of session-cookie rewrites while preserving the double-submit browser proof. The request marker keeps GraphQL programmatic and cross-origin setup paths simple while still distinguishing API calls from simple browser form posts.
**Tradeoff:** The frontend must attach the CSRF header for protected origin POSTs outside GraphQL. The token is not HttpOnly by design; it is not an auth secret, only a same-origin request proof paired with server-side signature verification. GraphQL clients must send the request marker when relying on cookie auth.
...
### 8a. OIDC provider registry and identity links

**Decision:** External login providers are configured as repeated OIDC-only `[[auth.providers]]` entries with stable local IDs, labels, client credentials, issuer URLs, optional scopes, optional email-scope requests, and provider options reserved for provider-specific expansion. OIDC verification remains on `go-oidc`; Goth and non-OIDC provider adapters are not part of the auth surface. Linked identities are durable `EVT` facts on the user aggregate. Pending first-time OIDC callbacks are short-lived `RUNTIME_STATE` records, not durable account facts.
**Why:** A provider list lets operators expose more than one OIDC login option without hard-coding a single provider. OIDC uses the verified issuer URL as the durable namespace because the issuer owns subject uniqueness; the configured provider ID is event-time metadata and can change without invalidating existing OIDC links. Matching by issuer plus subject survives provider-side email changes and avoids account takeover through recycled or unverified email addresses. Requiring user confirmation before JIT account creation avoids surprising users who expected to link an existing account.
**Tradeoff:** Existing `[auth.oidc]` TOML configuration, direct GitHub/GitLab/Google/Discord provider types, and legacy `CHATTO_AUTH_OIDC_*` environment variables are not accepted by this shape. Deployments should use `[[auth.providers]]` or counted `CHATTO_AUTH_PROVIDERS_<index>_*` variables; legacy OIDC env variables fail startup with a migration error instead of being silently ignored. OIDC providers that do not transmit email can still create passwordless local accounts, but users need a separate account-recovery path if they later lose access to the IdP.
