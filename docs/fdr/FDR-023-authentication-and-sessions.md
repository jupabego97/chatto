...
- **ADRs:** ADR-017 (cookie-session auth for WebSocket), ADR-024 (opaque bearer tokens for cross-origin auth), ADR-025 (multi-instance client architecture), ADR-033 (event-sourced state), ADR-036 (runtime state in `RUNTIME_STATE`), ADR-038 (shared projection consumers)
