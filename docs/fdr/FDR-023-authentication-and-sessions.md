...
- **Logout** — for registered bearer-token sessions, the client sends the token to `/auth/logout` so the server can revoke it and then removes the token locally. Cookie fallback logout deletes the current cookie-session KV record, clears the cookie, and the SPA does a hard reload.
